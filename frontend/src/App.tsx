...
function App() {
  ...
  // --- GÉNÉRATION DU TEXTE COMPLET (Pour l'affichage et la copie) ---
  ...
  // --- APPEL AU BACKEND (Pour l'export officiel via Jinja2) ---
  const downloadConfig = async () => {
    ...
    try {
      ...
      alert("Erreur: Le serveur FastAPI n'est pas lancé sur le port 8000.");
    }
  };
      ...
      <div className="relative z-10 max-w-7xl mx-auto w-full flex flex-col h-full">
        <div className="flex justify-between items-center mb-8 border-b border-gray-800 pb-4 shrink-0">
          ...
          <div className="flex gap-3">
            ...
            <button onClick={downloadConfig} className="bg-blue-600 border border-blue-500 px-4 py-1 rounded-md text-sm flex items-center gap-2 hover:bg-blue-500 transition-all">
              <Download size={16} /> Export (Jinja2)
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-stretch flex-1 min-h-0 pb-4">
                ...
                {Object.entries(config).map(([key, value]) => (
                  <div key={key} className="flex flex-col gap-1">
                    ...
                    <input
                      ...
                      className="bg-slate-900 border border-slate-700 rounded-md px-3 py-2 text-sm focus:ring-1 focus:ring-blue-400 outline-none transition-all text-gray-200"
                    />
                  </div>
                ))}
              </div>
          ...
          <div className="flex flex-col h-full min-h-0">
            <div className="bg-black rounded-[2rem] p-6 shadow-2xl border border-slate-800 flex flex-col h-full overflow-hidden">
              <p className="text-green-500 text-center mb-4 font-mono text-sm uppercase tracking-widest shrink-0">Output</p>

              <div className="flex-1 bg-[#0d0d0d] p-8 rounded-xl font-mono text-sm leading-relaxed text-gray-400 overflow-y-auto border border-slate-900 custom-scrollbar">
                <div className="space-y-1 whitespace-pre-wrap">
                  <p><span className="text-blue-400 font-bold">hostname</span> <span className="text-white">{config.hostname}</span></p>
                  <p className="text-gray-700">!</p>
                  <p><span className="text-blue-400 font-bold">username</span> <span className="text-white">{config.username}</span> privilege 15 secret <span className="text-red-500">********</span></p>
                  <p><span className="text-blue-400 font-bold">enable secret</span> <span className="text-red-500">********</span></p>
                  <p className="text-gray-700">!</p>

                  <p><span className="text-blue-400 font-bold">vlan</span> <span className="text-yellow-500">{config.vlan_admin_id}</span></p>
                  <p className="ml-5">name <span className="text-white">{config.vlan_admin_name}</span></p>
                  <p>exit</p>
                  <p className="text-gray-700">!</p>

                  <p><span className="text-blue-400 font-bold">interface Vlan</span><span className="text-yellow-500">{config.vlan_admin_id}</span></p>
                  <p className="ml-5 italic text-gray-500">description {config.vlan_admin_name}</p>
                  <p className="ml-5"><span className="text-blue-400 font-bold">ip address</span> <span className="text-emerald-400">{config.ip_admin}</span> {config.subnet_admin}</p>
                  ...
                  <p><span className="text-blue-400 font-bold">interface Port-channel</span><span className="text-yellow-500">{config.po_uplink_id}</span></p>
                  ...
                  {interfaces.map((iface, i) => iface.name && (
                    <div key={i} className="py-1">
                      <p><span className="text-blue-400 font-bold">interface</span> <span className="text-white">{iface.name}</span></p>
                      <p className="ml-5 italic text-gray-500">description {iface.description}</p>
                      <p className="ml-5">switchport</p>
                      <p className="ml-5">switchport mode trunk</p>
                      <p className="ml-5">switchport nonegotiate</p>
                      <p className="ml-5">channel-protocol lacp</p>
                      {iface.channel_group && <p className="ml-5">channel-group <span className="text-yellow-500">{config.po_uplink_id}</span> mode active</p>}
                      <p className="ml-5 text-gray-400">ip dhcp snooping trust</p>
                      <p>exit</p>
                      <p className="text-gray-700">!</p>
                    </div>
                  ))}

                  <p><span className="text-blue-400 font-bold">ip default-gateway</span> <span className="text-emerald-400">{config.ip_gateway}</span></p>
                  <p><span className="text-blue-400 font-bold">ip route</span> 0.0.0.0 0.0.0.0 <span className="text-emerald-400">{config.ip_gateway}</span> name Default-route</p>
                  <p className="text-gray-700">!</p>

                  <p><span className="text-blue-400 font-bold">ip domain name</span> <span className="text-white">{config.domain_name}</span></p>
                  <p><span className="text-blue-400 font-bold">ip ssh version</span> 2</p>
                  <p><span className="text-blue-400 font-bold">line vty 0 4</span></p>
                  <p className="ml-5">transport input ssh</p>
                  <p className="text-gray-700">!</p>
                  <p className="text-white">crypto key generate rsa modulus 2048</p>
